...
import { api, type NotePayload } from "../services/api";
...
export function usePromptBuilder() {
  ...
  const applySearchWorkspace = useAppStore((state) => state.applySearchWorkspace);
  ...
  const saveToLibrary = useCallback(async () => {
    if (!form.description) {
      toast.warning(t("promptBuilder.alerts.enterDescription"));
      return;
    }

    try {
      const categories = await api.getCategories();
      const promptCategory = categories.find((category) => category.system_key === 'prompt');
      if (!promptCategory) {
        toast.error(t("promptBuilder.alerts.promptCategoryMissing"));
        return;
      }
      ...
      const openSavedNote = async () => {
        try {
          const note = await api.getNote(result.note_id);
          navigate("/");
          openReading(note);
        } catch {
          toast.error(t("promptBuilder.alerts.openSavedNoteFailed"));
        }
      };
      const viewPromptLibrary = () => {
        ...
        applySearchWorkspace({
          searchQuery: "",
          selectedCategoryId: promptCategory.id,
          selectedTagId: null,
          sortBy: "updated",
          showArchived: false,
        });
      ...
      toast.success(t("promptBuilder.alerts.saved"), [
        { label: t("promptBuilder.actions.openSavedNote"), onClick: () => void openSavedNote() },
        { label: t("promptBuilder.actions.viewPromptLibrary"), onClick: viewPromptLibrary },
      ]);
    } catch (err) {

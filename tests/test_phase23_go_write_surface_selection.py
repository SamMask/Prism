...
def test_docs_record_23_3_completion_and_23_4_pending_gate():
    ...
    assert "Next active Go gate is `23.5 Go DB-only write expansion gate`, pending explicit approval" in architecture
    ...
    assert "`23.5 Go DB-only write expansion gate` is the next recommended step" in go_roadmap

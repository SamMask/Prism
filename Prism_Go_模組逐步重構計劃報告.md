...
`23.8 Local packaging execution track` is the next recommended step, pending explicit approval. The first subgate should be `23.8.1 Packaging contract`, defining Go binary, bundled frontend `dist`, external data dir, config/env/logs/uploads paths, SQLite WAL behavior, and the 23.7 Python-owned migration boundary.

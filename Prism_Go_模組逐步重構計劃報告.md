...
Phase 23 closes with retained Python as the normal runtime path. No Python removal, Go route ownership expansion, Caddy reload, systemd unit rewrite, frontend default API target change, Pillow removal, or public exposure expansion is authorized by Phase 23 closure. Future Go ownership expansion must start from a new explicit gate using the 23.10.1 retained-surface matrix. After `23.8-thumb.6`, `23.8-thumb.7 Go upload-url local implementation candidate` is the next thumbnail-specific gate before any Pillow removal can be reconsidered.

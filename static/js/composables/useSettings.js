...
export function useSettings() {
    ...
    // 快速新增預設分類 (v0.8.8)
    const quickAddDefaultType = ref(savedSettings.quickAddDefaultType || '提示詞');
    ...
    const availableThemes = [
        { id: 'default', name: '專業藍', nameEn: 'Professional Blue', color: '#3b82f6' },
        { id: 'cyberpunk', name: '賽博龐克', nameEn: 'Cyberpunk', color: '#e879f9' },
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => applyTheme(colorTheme.value));
    } else {
    ...
    const handleDeleteAllOriginals = async () => {
        const confirmMsg = t ? t('originalImages.confirmDelete', '確定要刪除所有原圖嗎？此操作無法復原，但筆記中的圖片路徑會自動改為縮圖。') : '確定要刪除所有原圖嗎？';
    ...
    const handleFixBrokenImages = async (onSuccess) => {
        const confirmMsg = t ? t('brokenImages.confirmFix', '確定要修正所有失效的圖片路徑嗎？') : '確定要修正所有失效的圖片路徑嗎？';
        if (!confirm(confirmMsg)) return;
        ...
        try {
            ...
            if (data) {
                const successMsg = t 
                    ? t('brokenImages.fixed', '已修正 {count} 個圖片路徑，更新了 {notes} 則筆記')
    ...
    const vacuumDatabase = async () => {
        if (!confirm(t('settings.vacuumConfirm', '確定要整理資料庫嗎？這可能需要幾秒鐘時間。'))) {
            return;
        }
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                const msg = t('settings.vacuumSuccess', '資料庫整理完成！')
    ...
    const catDelete = async (categoryId) => {
        ...
        // 如果沒有筆記，直接刪除
        if (noteCount === 0) {
            if (!confirm(t('messages.confirmDeleteCategory', `確定要刪除分類「${category.name}」嗎？`).replace('{name}', category.name))) {
                return;
            }
        ...
        // 建立選項清單
        const optionsList = otherCategories.map((c, i) => `${i + 1}. ${c.icon} ${c.name}`).join('\n');

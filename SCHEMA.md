...
> [!IMPORTANT] > **Schema 雙路徑架構說明**
>
> 資料庫結構定義存在於兩處 (這是**刻意設計**，非 bug)：
>
> | 路徑                     | 用途             | 執行時機         |
> | ------------------------ | ---------------- | ---------------- |
> | `app.py` `init_db()`     | 完整 Schema 建立 | 新資料庫首次啟動 |
> | `migrations/__init__.py` | 增量 Schema 變更 | 舊資料庫升級     |
>
> **維護規則**：
>
> - 新增欄位時，**必須同時更新兩處**
> - `init_db()` 保持最新完整 Schema
> - `migrations/` 新增對應的增量遷移腳本
>
> 此設計確保新舊資料庫都能正確運作。

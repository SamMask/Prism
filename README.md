...
| 版本                  | 適用對象         | 檔案大小 | 說明                       |
| --------------------- | ---------------- | -------- | -------------------------- |
| **完整版 (Portable)** | 一般使用者       | ~80MB    | 解壓即用，無需安裝任何軟體 |
| **輕量版**            | 已有 Python 環境 | ~8MB     | 需先安裝 Python 3.10+      |

> 💡 **推薦下載完整版**：`Prism_v*_Portable_*.zip`，雙擊 `start.bat` 即可使用！
...
| 軟體                                              | 必要性   | 說明                                            |
| ------------------------------------------------- | -------- | ----------------------------------------------- |
| [Python 3.10+](https://www.python.org/downloads/) | **必要** | 建議從官網下載，安裝時勾選 "Add Python to PATH" |
| Flask                                             | 必要     | `start.bat` 會自動安裝                          |
| Pillow                                            | 選用     | 已打包預編譯版，自動安裝（用於縮圖）            |

> ⚠️ **Microsoft Store 版 Python**：也支援！腳本會自動偵測 `py` 啟動器。

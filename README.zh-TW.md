...
目前 Windows portable executable 尚未做 code signing。從 GitHub 下載的 zip
可能帶有 Mark-of-the-Web (`Zone.Identifier`)，因此 Microsoft Defender
SmartScreen 可能顯示「Windows 已保護您的電腦」與「不明的發行者」。這是
unsigned portable build 的預期行為。

執行前請先用 GitHub Release notes 內列出的 SHA256 核對 V2.5 release zip：
...
| 工具 | 版本契約 |
|---|---|
| Go | Go 1.26.x；`go-shadow/go.mod` 宣告 `go 1.26.1`，本機 gate 以 Go 1.26.3 驗證 |
| Node.js | Node.js 22.14.0 |
| npm | npm 10.9.2 |
| Python | Python 3.11.x |
| pytest | `requirements.txt` 的 pytest 9.0.2 |
...
GitHub Actions baseline 使用 Windows runner，前端依賴以 `npm ci` 安裝，
Python 測試依賴由 `requirements.txt` 安裝，先跑 frontend build，再跑 Loop
verification gate；不依賴 SSH、Pi、production DB、uploads 或 private path。

...
- Note list lightweight gate 已完成 local + Pi delivery；2026-06-19 first-image hint regression 也已推到 Pi。`GET /api/notes` list payload 回 preview-compatible `content`、`content_preview`、`content_truncated`、`content_length`、`content_first_image`；`content_first_image` 只保留卡片/閱讀在無手動封面時用第一張圖的 fallback，不回傳全文。`GET /api/notes/<id>` 保持完整 note detail，搜尋仍可命中 preview 外的 tail content。Home card 使用 preview + full length metadata，Editor / card copy / image export 會先 lazy-load detail，ReadingView 維持 detail + auto-extracted attachment lazy-load。Pi live `https://prism.local` 已跑 Go primary cutover、API smoke 與 Playwright UI smoke；temp validation note/image 已清理。未新增 schema、cache、server-side UI state、全文預載、背景同步或附件 root 變更。
...
- Image viewer zoom follow-up 已完成 local gate：shared `ImageLightbox` 新增放大 / 縮小 / 重設縮放、ArrowUp / ArrowDown 快捷鍵與背景點擊關閉；切換圖片或重開 lightbox 回 100%/fit。ReadingView cover/markdown、EditablePreview、NoteEditor gallery 與 NoteCard cover 仍共用同一 viewer。Browser plugin `iab` 不可用，本輪用 Playwright fallback + 本機 Chrome + mocked API/static images 驗 local rendered flow；Loop gate 通過。未改 upload/delete/cleanup API、附件 contract、DB schema、gallery DB、per-image zoom persistence 或 Pi delivery。
...
- Deep scan risk closure 01A-01G 已完成 local gate：ReadingView / EditablePreview markdown render path 改用 shared DOMPurify sanitizer；文字附件 upload/read 統一 1 MiB hard limit；server backup download/rotate 改 SQLite consistent DB snapshot 並明確 DB-only；category invalid payload/delete target validation 回 400/404；bounded attachment body search 超限會回 optional partial diagnostics；no-auth/local-only boundary 補 regression；stability pack 覆蓋 search sync、中文/emoji、missing/Windows attachment path 與 bad pending-restore marker。未新增 auth、AI、schema、search index、備份平台、Pi deploy 或 Go runtime 大拆。Browser plugin `iab` 不可用時，rendered frontend 驗證使用 Playwright fallback + 本機 Chrome。
- PROJECT-REVIEW-HYGIENE-CANDIDATE-01 01A-01E 已完成 local gate：root `LICENSE` 補齊既有 MIT README 宣稱；新增 no-secret/no-Pi GitHub Actions baseline；requirements/docs/CI 對齊 Go 1.26.x、Node.js 22.14.0、npm 10.9.2、Python 3.11.x、pytest 9.0.2；新增 `docs/RELEASE_CHECKLIST.md` fresh evidence template；CONTRIBUTING E2E path 改為實際 `e2e/`。未新增 runtime/schema/API/auth/Pi deploy；第一次 push/PR 後仍需看 GitHub Actions 實際結果。
...
目前沒有未交付的 active construction item。若要發佈，下一步是依 `docs/RELEASE_CHECKLIST.md` 跑 commit / tag / package evidence：確認 commit scope 只含已完成且已驗證的功能、文件與測試收尾，再依 Lore Commit Protocol commit，視需要重建/確認 release package 與 tag；第一次 push/PR 後確認 GitHub Actions 實際綠燈。`DEEP-SCAN-RISK-CANDIDATE-01` 的 **01H Low-priority maintenance triage** 只有在要處理 bundle/browserslist warning、歷史 frozen docs/test wording 或 route-local Go 小整理時才 promote；installer / updater 只有在明確需要 Start Menu、桌面捷徑、指定資料夾 UI、WebView2 bootstrap、uninstall 或 update flow 時才另開 decision gate。不要自動開 AI、semantic search、installer 或 updater 實作。

...
下一個建議入口是 `READING-WORKSPACE-CANDIDATE-01 Reading list workspace`：先做純前端 state contract（建議 key `prism.readingWorkspace.v1`），支援多張 note 的暫存閱讀清單、切換、移除、清空與 scroll restore。不得新增 DB schema、server persistence、跨裝置同步、native 多視窗、diff engine 或改 `GET /api/notes` contract。若完成後要上 Pi，仍另跑 `DEPLOY-PI.md` 的 Pi delivery + live verification。installer / updater 只有在明確需要 Start Menu、桌面捷徑、指定資料夾 UI、WebView2 bootstrap、uninstall 或 update flow 時才另開 decision gate。

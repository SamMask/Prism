...
| CONTRACT-DESKTOP-SHELL-PORTABLE-PACKAGE | Desktop Shell Phase 4 只能產出免安裝 Windows portable zip / folder：`Prism.exe` GUI artifact、debug artifact、portable README、external data-dir default / first-run selector、`PrismPortable.json` persisted choice、clean-unzip smoke 與 no-bundled-DB guard；不得做 installer、auto updater、Pi deploy 或 production data bundling。 |

...
### 📌 Phase 23 status semantics rules — 候選 / owner / dependency 不得混用

> **狀態詞定義**：
> `candidate_done`：Go 已有候選實作或本機/copied-data 驗證；不代表正式 runtime 已使用，也不代表可移除 Python/Pillow。
> `live_owner`：正式 runtime / live route / packaged runtime 已由 Go 擁有，且通過相應 smoke、parity、rollback 驗證。
> `dependency_removed`：正式封裝與正常執行路徑已不再需要該依賴，例如 Python、venv、Pillow；不是只完成候選實作。
>
> **禁止偷換概念**：不得把 `candidate_done` 當成 `live_owner`；不得把 retained-Python stabilization 當成 `dependency_removed`；不得把 plan-only 文件當成交付完成；不得把 local/copied-data candidate 說成 live route owner；不得把 Go artifact build 成功說成 Python/Pillow removal 成功。

### ✅ 23.8-thumb Go WebP thumbnail ownership / Pillow removal track — Pillow Dependency Removal Closed

> **白話說明**：
> 這一步處理縮圖 dependency closure：正式 route owner 仍是 retained Python，但 Python route 產生縮圖時已委派 Go helper，不再依賴 Pillow。
> 目前實際狀態：`candidate_done: yes for thumbnail dependency closure`；`live_owner: retained Python routes delegate thumbnail generation to Go helper`；`dependency_removed: yes for Pillow`。
> 使用者看到的行為必須維持：支援 jpg/png/webp/gif 上傳、縮圖最大寬 500px、`_thumb.webp` 命名、`thumbnail_only` 成功時只回縮圖 URL、刪除/cleanup 仍能找到對應縮圖。
> 若下一步目標是封裝不含 Python，只能直接選 Python packaging removal roadmap B-E 大項之一交付；不得新增 decision gate、route-by-route planning gate、`23.8-thumb.8/9/10`。
...
- [x] **23.8-thumb.6** Go upload-url remote-fetch safety parity plan — 已完成 plan-only safety/parity contract：未改 runtime，先鎖 `POST /api/upload/url` 現行 Python contract（JSON `url`/`thumbnail_only`、http/https、SSRF、timeout/header、Content-Type、magic、size cap、filename、`thumbnail_only` 成功/失敗 fallback）與 Go candidate 必備條件。Go candidate 必須另用 `--enable-upload-url-write` / `PRISM_GO_ENABLE_UPLOAD_URL_WRITE`，預設 405 disabled，只寫 copied data dir，DB 維持 `sqlite_query_only=true`，並驗 redirect target、streaming size cap、failure no-mutation。見 `docs/contracts/phase23-go-upload-url-remote-fetch-safety-parity-plan.json`。
- [x] **23.8-thumb.7** Go upload-url local implementation candidate — 已完成 local/copied-data candidate：新增 `--enable-upload-url-write` / `PRISM_GO_ENABLE_UPLOAD_URL_WRITE` 的 Go `POST /api/upload/url`，預設 405 disabled，啟用後仍保持 DB `sqlite_query_only=true`，只寫 `PRISM_GO_DATA_DIR/static/uploads`。實作包含 http/https、literal IP + DNS SSRF guard、redirect target validation、30s timeout/header、Content-Type + magic validation、streaming size cap、sanitized basename / deterministic hash fallback、standard 原圖 + `_thumb.webp`、`thumbnail_only` 只寫縮圖，以及 thumbnail 失敗保留原圖 fallback。Windows build、linux/arm64 CGO=0 build 與 Go unit tests 已通過。狀態語義：`candidate_done: partial yes`；`live_owner: no`；`dependency_removed: no`。本輪未移除 Pillow、未改 Python upload-url/import helper/delete/cleanup owner、未部署 Pi、未改 Caddy/systemd/frontend default。見 `docs/contracts/phase23-go-upload-url-local-candidate.json`。
- [x] **Pillow dependency removal closure** — 已完成唯一縮圖收尾項：`requirements.txt` 與 `requirements-pi.txt` 均不含 Pillow；Python upload/import routes 不再 import PIL，並透過 `utils/image_tools.py` 呼叫 Go thumbnail helper；`scripts/start.bat` 不再檢查/安裝 PIL；`docs/SEQUENCE-UPLOAD.md` 與 README 不再把 Pillow 寫成 runtime 依賴；`scripts/smoke_go_local_artifact.ps1` 會用 built Go artifact 直接把 PNG 轉成 `_thumb.webp` 並驗 RIFF/WEBP signature，證明 fresh packaged runtime 的縮圖 helper path 不需安裝 Python/Pillow。狀態語義：`candidate_done: yes for thumbnail dependency closure`；`live_owner: retained Python routes delegate thumbnail generation to Go helper`；`dependency_removed: yes for Pillow`。本輪未移除 Python backend、未擴大 live Go HTTP upload route ownership、未部署 Pi、未改 Caddy/systemd/frontend default。見 `docs/contracts/phase23-pillow-dependency-removal-closure.json`。
- **收斂決議**：23.8-thumb 在 23.8-thumb.7 後只剩上面的 `Pillow dependency removal closure`，不再自動新增 `23.8-thumb.8/9/10`。這條線的目標是解除 Pillow thumbnail 封裝依賴，但不得把 candidate completion、live owner、dependency removal 混成同一件事。
- **A 完成標準**：已完成。所有封裝 runtime 會產生 `_thumb.webp` 的路徑不再需要 Pillow；`POST /api/upload`、`POST /api/upload/url`、import helper 仍由 retained Python route 擁有，但縮圖生成已收斂為 Go helper；upload delete / cleanup 對 `_thumb.webp` companion 行為由既有 parity fixtures 覆蓋；`requirements.txt` / `requirements-pi.txt` 均移除 Pillow；Python runtime 無 `from PIL import Image` / `PIL_AVAILABLE` fallback wording / startup Pillow check；API/docs/sequence/README 不再宣稱 Pillow 是 runtime dependency；Go unit tests、Windows build、linux/arm64 `CGO_ENABLED=0` build、thumbnail parity fixtures、Go local artifact smoke、full `pytest tests/ -v` 為本項收尾驗證。
...
- [x] **23.9.3** Live verification — live 驗 `/api/test` status ok notes 198 / categories 6 / tags 128、`/api/server/version` v2.5 Linux V2 mode true、migration current/latest v16 pending `[]`、`/api/system/go-read-routing` enabled false / default_owner python、`/api/notes?per_page=1` success total 198；headers 無 `X-Prism-Go-Read-Routing`，journal 顯示 v16 最新與 port 5000，未見新 write/error。
...
### ✅ 23.10 Python reduction and final stabilization — Retained-Python Final State Complete

> **白話說明**：
> 這一步已完成的是 retained-Python final state，不是 Python removal。
> 23.10 retained-Python stabilization 不是 Python removal，也不是 pure-Go packaging completion。
> 它的結論是：當時 critical surfaces 尚未全數 Go-owned，所以正式 runtime 保留 Python。
> 若下一輪目標改成真的去掉 Python 封裝，不能再從 23.10 宣稱完成；必須走下面「Python packaging removal roadmap」的大項。
>
> 原本意圖：決定 Python 還剩什麼、能不能移除、以及最終 Go runtime 是否完整。
...
- [x] **23.10.3** Final stabilization window — 已完成 retained-Python final state stabilization：本機通過 frontend typecheck/build、Go local artifact smoke、local API smoke、Playwright browser screenshot smoke；Pi 在 `PI5Mask24` 建立 DB/data/Caddy backups 後，以 tar-over-SSH 同步 current worktree（排除 production DB/uploads/attachments/env/log/build），重啟既有 `prism.service`，live 驗 service/Caddy active、Caddy validate、version 2.5、migration v16 clean、Go routing disabled/Python owner、notes total 198、headers 無 Go routing header、journal restart 後無新錯誤。見 `docs/contracts/phase23-final-stabilization.json`。

### ⏭️ Python packaging removal roadmap — 大項到可去掉 Python，不再拆碎

> **白話說明**：
> 目標是讓正式封裝不再需要 Python / venv；Pillow dependency removal 已在 A 完成。
> 這裡只列大項，不再把每條 route 拆成一串新的細項。
> 每個大項都必須一次交付「缺口、補完、驗證、刪除或保留決策」；不能用下一個小 gate 取代完成標準。
> **目前白話結論**：
> A-E 已經跑完，但沒有達成「可以刪 Python」；只達成「Pillow 可以刪、Go packaged runtime 有候選、Python package 目前不能刪」。
> D 不能永久 Go cutover 的直接原因是：Go 現在 live 只適合接 read/status route：`/api/test`、`/api/categories`、`/api/tags`、`GET /api/notes`、`GET /api/system/migration-status`。
> upload / upload-url / delete / cleanup / import / export / server / migration-runner 還是 Python owner；如果把 Caddy 或 systemd 永久全量切到 Go，正式工作流會壞。
> 所以 E 的「完成」不是 Python package deletion 完成，而是 deletion decision 完成：**不能刪 Python package，必須保留 retained-Python runtime**。
> Risk level: `P0 safety-critical`。

- [x] **A. Pillow dependency removal closure** — 已完成上方唯一縮圖收尾項；Pillow 已從 Windows/Pi Python dependency manifests 移除，Python route 縮圖生成委派 Go helper，fresh local packaged artifact 以 `--thumbnail-input` / `--thumbnail-output` 產生 `_thumb.webp`。未移除 Python backend，也未擴大 live Go HTTP route ownership。
- [x] **B. Runtime ownership closure for Python removal — final retained-Python closure (2026-06-06)**：B 已結案，不再新增或自動排 `B-next.*`。結論不是 Python removal ready；結論是目前 no-Python packaged runtime 不成立，正式 runtime owner 保留 Python Flask。原因：Pillow dependency removal 已完成且 Go 有 local candidates，但一般 Prism workflow 仍依賴 Python live writes、uploads/attachments/delete/cleanup、import/export、prompt/wizard options、migrations/schema/db maintenance、frontend static serving 與 rollback owner。簡易說明：要繼續追 no-Python 會變成大規模產品/runtime cutover，不是 B 裡可以靠繼續拆細項自然完成；因此 B 收成 retained-Python final decision。見 `docs/contracts/phase23-python-runtime-ownership-closure.json`。C/D/E 不從 B 自動啟動；C 是後續本輪明確授權。
- **B-next.1 Notes write/actions/history/batch packaged-runtime ownership bundle — historical completed_local_candidate (2026-06-06)**：新增 `--enable-notes-write` / `PRISM_GO_ENABLE_NOTES_WRITE=1`，Go local/copied-DB candidate 已覆蓋 notes create/update/delete、pin/archive、duplicate/reorder、batch type/tags/delete、history list/restore/delete-history，並以 Python-vs-Go response + DB-state parity、default disabled 405、local artifact smoke 鎖定。見 `docs/contracts/phase23-b-next-notes-write-bundle.json`。此項保留為證據，不再導向新的 B-next 細項；live/default owner 仍是 Python。
- **B-next.2 Notes bundle live/default ownership decision and media-cleanup boundary — ✅ completed_decision_no_promotion (2026-06-06)**：決策為不把 B-next.1 升成 live/default owner。原因：B-next.1 只有 local/copied-DB parity，notes delete / batch delete 仍由 Python `_cleanup_note_images` 負責引用計數後的 uploads / `_thumb.webp` 檔案清理，且本項未授權 Pi、Caddy、systemd、frontend default、production DB 或 rollback rollout。簡易說明：Go 現在能在測試 DB 改 notes，但正式封裝若刪筆記還要安全刪除或保留圖片檔，不能用 DB parity 假裝完成。見 `docs/contracts/phase23-b-next-notes-live-default-media-boundary.json`。No B-next.3 added；B final closure 後不再從這裡延伸任何自動細項。
- **B no-Python gaps closed as retained Python（非下一步）**：`category_tag_live_writes`、`uploads_attachments_raw_delete_cleanup`、`import_export_long_content`、`migrations_schema_db_maintenance`、`frontend_static_default_rollback_owner` 都不再作為自動 queue；它們是 retained-Python runtime responsibilities。若未來重新啟動 no-Python，必須另開一個明確授權的大型 cutover/implementation scope，不能從 B 自動延伸。
- [x] **C. Go packaged runtime release candidate — completed_release_candidate (2026-06-06)**：已產出不含 Python/venv 的 Windows `build/go-runtime/prism-go-runtime.exe` 與 linux/arm64 `build/go-runtime/prism-go-runtime-linux-arm64` Go artifacts；`scripts/smoke_go_local_artifact.ps1` 使用 external data dir 與 copied DB/data under `build/go-local-smoke/`，驗 health、embedded SPA、read APIs、Go `GET /api/system/migration-status`、default disabled writes、explicit copied-DB tag/category/notes writes、explicit copied-data attachment text file read、以及 built artifact `--thumbnail-input` / `--thumbnail-output` 產生 `_thumb.webp`。source `knowledge.db` SHA256 guard 確認原 production DB 不被改寫；本輪未部署 Pi、未改 Caddy/systemd/frontend default、未移除 Python、未啟動 D/E、未新增 C-next。見 `docs/contracts/phase23-go-packaged-runtime-release-candidate.json`。
- [x] **D. Live cutover and rollback proof — completed_rollback_proof_no_permanent_cutover (2026-06-06)**：已在 `PI5Mask24` 做受控 live proof 並回滾。先備份 DB/data/Caddy/Python service/Go sidecar unit/Go sidecar binary；短暫替換 Go sidecar 為 C artifact 並把 Caddy 既有 Go read matcher 暫時擴到 `GET /api/system/migration-status`；live 驗 `/api/test`、categories、tags、notes read、migration-status 皆有 `x-prism-go-read-routing: hit`，server version、JSON/Markdown export、upload/import unsafe probe 仍由 Python/Werkzeug 回應且無 Go routing header。rollback 後還原 pre-D Caddyfile 與 pre-D sidecar binary，migration-status 回 Python，`knowledge.db` SHA256 與 pre-D backup 相同，`prism.service` / Caddy / `prism-go-readonly.service` active。結論：D 證明可受控切換與回滾，但不做永久全量 Go cutover；因 upload/delete/cleanup/import/export/server/migration-runner 仍是 retained-Python owner。見 `docs/contracts/phase23-go-live-cutover-rollback-proof.json`。No D-next added。
- [x] **E. Python package deletion — completed_no_deletion_retained_python_package (2026-06-06)**：E 已收尾，結論是不刪 Python package/runtime。原因：D 的結果是 `completed_rollback_proof_no_permanent_cutover`，正式 normal runtime owner 仍是 Python `prism.service`，而 upload/delete/cleanup/import/export/server/migration-runner 還是 Python-owned；若刪 `app.py`、`routes/`、`migrations/`、Flask requirements、Pi `linux-venv` install path、PyInstaller/portable/start scripts，會破壞目前已驗證的 retained-Python runtime。Pillow 已在 A 移除，`requirements.txt` / `requirements-pi.txt` 仍保留 Flask、requests、python-magic 系列作為 Python runtime dependency。見 `docs/contracts/phase23-python-package-deletion-closure.json`。No E-next added。
- **執行規則**：下一輪若目標是去掉 Python，只能從 A-E 大項選一個交付，不得再新增 `23.8-thumb.8` 或 route-by-route planning gate 來替代交付。
...
| 版本 | 日期 | 內容 |
|------|------|------|
| **go-roadmap** | 2026-06-06 | C Go packaged runtime release candidate — 在明確授權後完成單一 C closure，未新增 C-next：新增 Go `GET /api/system/migration-status` read-only endpoint；`scripts/smoke_go_local_artifact.ps1` 現在驗 Windows + linux/arm64 artifacts、migration-status、default disabled writes、copied-DB notes/tag/category writes、copied-data attachment text file read、thumbnail helper `_thumb.webp` 與 source DB SHA256 guard。新增 `docs/contracts/phase23-go-packaged-runtime-release-candidate.json` 與 regression lock。本輪未部署 Pi、未改 Caddy/systemd/frontend default、未寫 production DB/files、未移除 Python、未啟動 D/E。 |
| **go-roadmap** | 2026-06-06 | B final retained-Python closure — 依使用者要求把 B 結尾，不再新增或自動排 `B-next.*`。`docs/contracts/phase23-python-runtime-ownership-closure.json` 狀態改為 `completed_final_retained_python_closure`：Python removal 仍不 ready，但 B 本身結案為 retained-Python final decision；Go notes bundle 保留為 local/copied-DB candidate evidence，B-next.2 保留為不升 live/default owner decision。C/D/E 不從 B 自動啟動；C 若執行必須另行明確授權。 |
| **go-roadmap** | 2026-06-06 | B-next.2 Notes bundle live/default ownership decision and media-cleanup boundary — 在明確授權後完成 notes bundle 升級決策：不把 B-next.1 local/copied-DB candidate 升成 live/default owner。原因是 notes delete / batch delete 仍有 Python `_cleanup_note_images` media cleanup 檔案副作用，且未授權 Pi/Caddy/systemd/frontend default/production DB/rollback rollout。新增 `docs/contracts/phase23-b-next-notes-live-default-media-boundary.json` 與 regression lock；No B-next.3 added，且 B final closure 後不再從這裡延伸任何自動細項。 |
| **go-roadmap** | 2026-06-06 | B-next.1 Notes write/actions/history/batch packaged-runtime ownership bundle — 在明確授權後完成 Go local/copied-DB notes bundle candidate：新增 `--enable-notes-write` / `PRISM_GO_ENABLE_NOTES_WRITE=1`，覆蓋 notes create/update/delete、pin/archive、duplicate/reorder、batch type/tags/delete、history list/restore/delete-history。新增 `docs/contracts/phase23-b-next-notes-write-bundle.json` 與 Python-vs-Go response + DB-state parity regression；local smoke artifact 現在驗 default notes write 405，以及 explicit notes-write copied DB create/update/history/delete。B 仍未完成：live/default ownership、media cleanup side effects、Pi/Caddy/systemd/frontend default、Python removal、C/D/E 仍 blocked。 |
| **go-roadmap** | 2026-06-06 | Runtime ownership closure for Python removal (B) — 在明確授權後執行 B closure assessment，新增 `docs/contracts/phase23-python-runtime-ownership-closure.json` 與 regression lock。原 assessment 證明 Pillow 已移除，但 notes writes/actions/history/batch、category/tag live writes、uploads/attachments/delete/cleanup、import/export/long-content、prompt/wizard options、migrations/schema/db maintenance、frontend static/rollback owner 仍是 request-time Python blockers；後續已由 B final retained-Python closure 收尾，不再作為 active next queue。 |
| **go-roadmap** | 2026-06-06 | Pillow dependency removal closure — 在明確授權後完成 23.8-thumb / Python packaging removal roadmap A：`requirements-pi.txt` 移除 Pillow；`requirements.txt`、Python upload/import routes、`scripts/start.bat`、`docs/SEQUENCE-UPLOAD.md` 已保持無 PIL/Pillow runtime dependency；README 技術堆疊改為 Go WebP thumbnail helper；`scripts/smoke_go_local_artifact.ps1` 新增 built Go artifact `--thumbnail-input` / `--thumbnail-output` smoke，直接產生 `_thumb.webp` 並驗 RIFF/WEBP signature。狀態為 `candidate_done: yes for thumbnail dependency closure`、`live_owner: retained Python routes delegate thumbnail generation to Go helper`、`dependency_removed: yes for Pillow`。本輪未移除 Python backend、未擴大 live Go HTTP upload route ownership、未部署 Pi、未改 Caddy/systemd/frontend default；下一步若要繼續 Python packaging removal，直接選 B-E 大項。 |
| **go-roadmap** | 2026-06-06 | Phase 23.8-thumb status semantics correction — 依實際 source 修正 candidate / live owner / dependency removal 語義：23.8-thumb.7 只能算 upload-url thumbnail local/copied-data `candidate_done: partial yes`，不是 Go live owner，也不是 Pillow dependency removal。Pillow 目前 `candidate_done: partial yes`、`live_owner: no`、`dependency_removed: no`；雖然 `requirements.txt`、upload/import routes、`scripts/start.bat` 與 sequence wording 已有部分 Pillow 清理，但 `requirements-pi.txt` 仍含 Pillow，fresh packaged runtime 不安裝 Python/Pillow 也能產生 `_thumb.webp` 尚未證明。23.10 保持 retained-Python final state，不是 Python removal / pure-Go packaging completion。 |
| **go-roadmap** | 2026-06-06 | Python packaging removal roadmap correction — 依使用者要求把 `23.8-thumb` 收斂成單一 `Pillow dependency removal closure`，明確列出刪 Pillow 前缺口、補完、驗證、刪除條件；另新增「Python packaging removal roadmap」A-E 大項，直接規劃到可去掉 Python/venv/Pillow 的程度。規則固定：下一輪若目標是去掉 Python，只能從 A-E 大項選一個交付，不得再新增 `23.8-thumb.8` 或 route-by-route planning gate 來替代交付。本輪為 TODO/contract/docs regression lock 修正，不改 runtime code。 |
...
| **go-roadmap** | 2026-06-06 | Phase 23.8-thumb.7 Go upload-url local implementation candidate — 在明確授權後新增 flag-gated local/copied-data Go upload-url candidate：`--enable-upload-url-write` / `PRISM_GO_ENABLE_UPLOAD_URL_WRITE` 啟用 `POST /api/upload/url`，預設 405 disabled；啟用後只寫 `PRISM_GO_DATA_DIR/static/uploads`，DB 維持 `sqlite_query_only=true`。實作鎖住 http/https、literal IP + DNS SSRF guard、redirect target validation、30s timeout/header、Content-Type + magic validation、streaming size cap、sanitized filename/hash fallback、standard 原圖 + `_thumb.webp`、`thumbnail_only` 只寫縮圖、thumbnail 失敗保留原圖。`go test ./...`、Windows build、linux/arm64 CGO=0 build 已通過。本輪未改 Python upload-url/import/delete/cleanup owner、未移除 Pillow、未部署 Pi、未改 Caddy/systemd/frontend default。狀態只到 `candidate_done: partial yes`；`live_owner: no`；`dependency_removed: no`。23.8-thumb 不自動新增下一個 thumbnail gate；真正封裝解除只能由 Python packaging removal roadmap A-E 大項交付。 |
...
| **go-roadmap** | 2026-06-06 | Phase 23.9 Pi deployment rollout — 在明確授權後完成 Pi preflight、受控同步與 live verification：`prism.service` / Caddy active+enabled、Caddy validate passed、migration current/latest v16 pending `[]`；建立 DB backup `prism_pre_23_9_rollout_20260606_015426.db`、data snapshot `prism_pre_23_9_data_snapshot_20260606_015426.tar.gz`、Caddy backup `Caddyfile.prism-pre-23-9-rollout-20260606_015426.bak`。用 tar-over-SSH 同步目前 worktree，排除 production DB/WAL/SHM、uploads、attachments、`.port_config`、env/log/build artifacts，重啟既有 `prism.service`。Live 驗 `/api/test` notes 198 / categories 6 / tags 128、`/api/server/version` v2.5 Linux V2 mode true、migration v16 pending 空、`/api/system/go-read-routing` enabled false / Python owner、`/api/notes?per_page=1` total 198；headers 無 Go routing header，journal 無新 write/error。本輪未改 Caddyfile、未 reload Caddy、未改 systemd unit、未擴 Go route ownership、未寫 production DB/files、未移除 Python、未做 Pillow removal 或 Go WebP encoder。下一個 Go runtime gate 為需另行明確授權的 23.10.1 ownership closure audit；縮圖線仍是獨立的 23.8-thumb.1。 |
...
| **pi-deploy** | 2026-05-28 | 部署 Phase 18.3 follow-up 到 Raspberry Pi：同步 `frontend/dist`、`utils/query_builder.py`、`docs/TODO.md`，重啟 `prism.service` 後 live 驗證 `active`；`/api/test` status ok、`/api/server/version` v2.5 + V2 mode true、`/api/system/migration-status` current/latest 15 且無 pending；dist assets 為 `index-5Z_Y_Vm1.js` / `index-BqlIPmnX.css`；建立臨時 Pi 筆記後 `/api/notes?q=todo.md&per_page=100` 命中並成功刪除臨時資料。 |

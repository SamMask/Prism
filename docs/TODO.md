...
## i18n（多語系）還原 — 待辦（2026-06-14 記錄）

**目標**：中(zh-TW)/英(en)/日(ja)/韓(ko) 四語，於 `設定 > 外觀` 即時切換、localStorage 持久化。

**現況盤點**（`frontend/src/i18n/index.ts`）
- 已有 `t(key, params)` / `setLocale` / `getLocale` / `initLocale` / `availableLocales` 與 zh-TW、en 兩份字典。
- **致命缺陷：`currentLocale` 是模組級變數**，`t()` 讀它但改它**不會觸發 React 重繪** → 現在切語系畫面不會更新。這是還原的第一順位。
- 字典只覆蓋少量 key（common/note/settings/ai/history/pin），且 `ai` 區塊已過期（AI 在 v2.3.0 移除）。
- **全專案 0 個元件實際呼叫 `t()`**——UI 字串全是寫死的繁中。

**施工順序（建議分階段，避免一次全改）**
- [ ] **階段 1 — 修反應式根**：把 locale 從模組變數移進反應式狀態（優先用既有 `useAppStore` Zustand store，或新建 `LanguageProvider` Context）。提供 `useTranslation()` hook 讓元件訂閱 locale 變更而重繪。`t()` 仍可保留給非元件場景，但元件一律走 hook。
- [ ] **階段 2 — 切換器 + 持久化**：`AppearanceSection`（設定 > 外觀）加語系下拉，用 `availableLocales`（擴成 4 語）。沿用 localStorage、app 啟動 `initLocale()`。先在「只有少數已 key 化字串」上驗證即時切換可動。
- [ ] **階段 3 — 字串抽取**：把寫死的繁中逐區塊抽進字典 key（建議按頁/元件分 namespace：editor、search、promptBuilder、settings、serverDashboard、backup/restore…）。移除過期的 `ai` 區塊。這是工量最大、最該分批 PR 的部分。
- [ ] **階段 4 — 補 ja/ko 字典**：依抽取後的完整 key 集補日、韓翻譯。
- [ ] **階段 5 — 收尾**：缺漏 key 的 fallback 行為（目前回 key 字串 + console.warn）、`{count}` 參數替換已支援需保留、考慮對 key 加型別約束避免拼錯。

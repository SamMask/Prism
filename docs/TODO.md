...
- [ ] **23.8-thumb.5** Go thumbnail surface expansion or removal-readiness gate — 下一步若授權，決定是否把 Go candidate 擴到 `POST /api/upload/url` 與 import helper，或明確保留這些 Python surfaces；在此之前仍不得移除 Pillow 或把 Go upload route 接到 Pi/Caddy/frontend default。
- **收尾驗證**：`cd go-shadow && go test ./...`、Windows build、linux/arm64 `CGO_ENABLED=0` build、`pytest tests/test_phase23_go_thumbnail_local_candidate.py -v`、`pytest tests/test_phase23_go_thumbnail_parity_and_pillow_removal_gate.py -v`、Phase 23 regression set、`pytest tests/ -v`。

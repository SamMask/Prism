...
- [ ] **D. Live cutover and rollback proof** — 在 Pi / Caddy / systemd 層做受控切換與 rollback proof：先備份 DB/data/Caddy/service config，再切 Go runtime，驗核心 API、uploads/thumbnails/import/export、migration status、journal/error、routing headers；rollback 能回到前一個 retained-Python 狀態。

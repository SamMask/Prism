...
### ⏭️ 23.8-thumb Go WebP thumbnail ownership / Pillow removal track — Candidate Partial, Dependency Removal Pending

> **白話說明**：
> 這一步處理縮圖 owner：Go 已有 upload / upload-url thumbnail local/copied-data candidates，但這不等於 Go 已是正式 live route owner。
> 目前實際狀態：`candidate_done: partial yes`；`live_owner: no`（正式 upload / upload-url / import helper route owner 仍是 Python）；`dependency_removed: no`（`requirements-pi.txt` 仍含 Pillow，fresh packaged runtime 不安裝 Python/Pillow 仍可產生 `_thumb.webp` 尚未證明）。
> 使用者看到的行為必須維持：支援 jpg/png/webp/gif 上傳、縮圖最大寬 500px、`_thumb.webp` 命名、`thumbnail_only` 成功時只回縮圖 URL、刪除/cleanup 仍能找到對應縮圖。
> 若下一步目標是封裝不含 Python/Pillow，只能直接選 Python packaging removal roadmap A-E 大項之一交付；不得新增 decision gate、route-by-route planning gate、`23.8-thumb.8/9/10`。
...
- [ ] **Pillow dependency removal closure** — 唯一縮圖收尾項仍未完成。實際部分清理已存在：`requirements.txt`、Python upload/import routes、`scripts/start.bat` 與 `docs/SEQUENCE-UPLOAD.md` 已不再直接引用 PIL/Pillow；但 closure 完成標準尚未滿足，因 `requirements-pi.txt` 仍含 `Pillow`，且 fresh packaged runtime 不安裝 Python/Pillow 也能產生 `_thumb.webp` 尚未證明。狀態語義：`candidate_done: partial yes`；`live_owner: no`；`dependency_removed: no`。見 `docs/contracts/phase23-pillow-dependency-removal-closure.json`。
...
- **A 完成標準**：所有封裝 runtime 會產生 `_thumb.webp` 的路徑不再需要 Python/Pillow；`POST /api/upload`、`POST /api/upload/url`、import helper 的 thumbnail ownership 或封裝策略明確收斂；upload delete / cleanup 對 `_thumb.webp` companion 行為通過既有 parity fixtures；移除 `requirements.txt` / `requirements-pi.txt` 的 Pillow dependency；移除 Python runtime 的 `from PIL import Image` 依賴、`PIL_AVAILABLE` fallback wording、啟動腳本 Pillow check、API/docs/sequence docs 中會讓人以為封裝仍依賴 Pillow 的描述；Go unit tests、Windows build、linux/arm64 `CGO_ENABLED=0` build、thumbnail parity fixtures、Go local artifact smoke、full `pytest tests/ -v` 通過；證明 fresh packaged runtime 不安裝 Python/Pillow 也能產生 `_thumb.webp`。
...
> **白話說明**：
> 目標是讓正式封裝不再需要 Python / venv / Pillow。
> 這裡只列大項，不再把每條 route 拆成一串新的細項。
> 每個大項都必須一次交付「缺口、補完、驗證、刪除或保留決策」；不能用下一個小 gate 取代完成標準。
> Risk level: `P0 safety-critical`。

- [ ] **A. Pillow dependency removal closure** — 上方唯一縮圖收尾項；目前 `candidate_done: partial yes`、`live_owner: no`、`dependency_removed: no`。完成時必須一次交付缺口、補完、驗證、刪除或明確保留決策，不得用下一個 decision gate / route-by-route gate / `23.8-thumb.*` 取代完成標準。
- [ ] **B. Runtime ownership closure for Python removal** — 用 23.10.1 retained-surface matrix 一次收斂：notes write/actions/history/batch、categories/tags live writes、uploads/attachments raw/delete/cleanup、import/export、system/server/config、migrations、static serving、rollback owner 必須各自是 Go-owned、移除、或明確降級成「不進封裝 runtime」的外部 maintenance tool。完成標準：沒有 request-time / packaged-runtime surface 仍要求 Python backend。

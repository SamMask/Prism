...
- [ ] **E. Python package deletion** — 只有 A-D 通過後，才移除 Python backend 封裝路徑、venv/PyInstaller/start script Python runtime assumptions、Pillow 與不再使用的 Python runtime deps；保留的 Python 檔若只是 maintenance tool，必須在 docs 標成非封裝 runtime。完成標準：fresh checkout/build 不安裝 Python deps 也能產出可用 runtime artifact，full tests / package smoke / Pi smoke 通過。

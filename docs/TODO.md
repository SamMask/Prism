...
- [ ] **C. Go packaged runtime release candidate** — 產出不含 Python/venv 的 Windows 與 linux/arm64 Go artifact；使用 external data dir、既有 DB/uploads/attachments/config/logs；支援 migration-status/health/read/write/file/thumbnail 的封裝 smoke；原 production DB/uploads 不被 build/smoke 改寫。

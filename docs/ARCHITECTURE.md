...
- Next active Go gate is `23.6-next First Go file-read route implementation candidate`, pending explicit approval. The selected candidate is only the text JSON branch of `GET /api/attachments/<attachment_id>` against local/copied DB + copied `docs/attachments`; `raw=true`, binary/send_file, upload/delete, cleanup, import/export, server backup/logs, live routing, production DB/files, Pi deploy, schema, and Python removal remain blocked.

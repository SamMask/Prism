...
This section is the structural basis for the active Go replacement roadmap in `docs/TODO.md`. It does not claim that Go is already the production owner. Current production ownership remains retained Python until every target boundary below is implemented and verified.
...
| ARCH-GO-PRIMARY-08 | Security | Go must preserve local/public exposure boundaries and block path traversal, SSRF, unsafe MIME, unsafe size, and unauthenticated public deployment risks. |

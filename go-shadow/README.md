...
`go-shadow/main_test.go::TestNotesDeleteCleansImagesFTSAndAssociations` locks the active-roadmap T013 notes delete/batch-delete parity, DB/FTS association cleanup, static/uploads original + `_thumb.webp` cleanup, unshared attachment file cleanup, shared attachment file preservation, docs status, and non-live-promotion boundary.
